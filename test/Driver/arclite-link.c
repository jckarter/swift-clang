...
// RUN: %clang -### -target arm64-apple-ios -fobjc-link-runtime -lfoo -mios-version-min=8.0 %t.o 2>&1 | FileCheck -check-prefix=CHECK-ARCLITE-IOS %s
// CHECK-ARCLITE-IOS: libarclite_iphoneos.a

// RUN: %clang -### -target x86_64-apple-ios -fobjc-link-runtime -mios-version-min=8.0 %t.o 2>&1 | FileCheck -check-prefix=CHECK-ARCLITE-IOSSIM %s
// CHECK-ARCLITE-IOSSIM: libarclite_iphonesimulator.a

// RUN: %clang -### -target armv7-apple-tvos -fobjc-link-runtime -mtvos-version-min=8.0 %t.o 2>&1 | FileCheck -check-prefix=CHECK-ARCLITE-TVOS %s
// CHECK-ARCLITE-TVOS: libarclite_appletvos.a

// RUN: %clang -### -target i386-apple-tvos -fobjc-link-runtime -mtvos-version-min=8.0 %t.o 2>&1 | FileCheck -check-prefix=CHECK-ARCLITE-TVSIM %s
